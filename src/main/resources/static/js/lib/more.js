...
$(function(){
  ...
  var search_size = utils.getParam('fontSize'),          //- 字体
      ...
      search_font = utils.getParam('fontName') ? utils.getParam('fontName') : '方正字体',          //- 查找字体名称
  ...
  var $fontList = $('.font-cont').find('li'); //- 更多字体列表
  ...
  var request = function () {
    var turl = './more.html?fontName=' + search_font;
    // 发起请求
    if (search_size !== '') {
  ...
  // 遍历字体
  var loopFont = function (s_font) {
    $fontList.map(function(i,k){
      var unf = $(k);
      var unindex = $(k).parent().index();
      if (unf.find('a').html() === s_font) {
        unf.addClass('active').siblings().removeClass('active');
        unf.parent().addClass('active').siblings().removeClass('active');
        $('.font-tab').find('span').eq(unindex).addClass('active').siblings().removeClass('active');
      }
    });
  }
  ...
  // 初始化
  if (search_font) {
    loopFont(search_font);
    document.title = search_font + '下载-' + search_font + '预览-' + search_font + '库-更多字体列表-字体下载-求字体网'
  } else {
  ...
  // 更多字体列表切换 按钮
  $('.font-tab span').on('click', function(){
    var obj = $(this);
    var index = $(this).index();
    if (!obj.hasClass('active')) {
      obj.addClass('active').siblings().removeClass('active');
      $('.font-cont ul').eq(index).addClass('active').siblings().removeClass('active');
    }
    return false;
  });

  // 更多字体选择 按钮
  $('.font-cont li').on('click', function(){
    var obj = $(this);
    var fn_ch = obj.find('a').html();
    if (!obj.hasClass('active')) {
      $('.search-input').val('');
      // loopFont(fn_ch);
      search_font = fn_ch;
      search_page = 0;
      search_previewText = $.trim($('.preview-input').val());
